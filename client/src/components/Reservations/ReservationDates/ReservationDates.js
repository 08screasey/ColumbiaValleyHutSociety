...
const ReservationDates = (props) => {
    ...
    useEffect(() => {
        if (props.admin && props.editing) {
            setRange({
                from: new Date(props.editing.dates[0]),
                to: new Date(props.editing.dates[props.editing.dates.length - 1]),
            });
        }
    ...
    const handleDayClick = (day, { disabled, unavailable, editing }) => {
        ...
        } else if (range.from && days_between(range.from, day) > props.bookedDates[cabin].maxNights - 1) {
            setError(`The maximum stay length is ${props.bookedDates[cabin].maxNights} days`);
        } else {
    ...
    const unavailableChecker = (date1, date2) => {
        ...
        dateRange.forEach((date) => {
            if (flag) {
                ...
                flag = props.bookedDates[cabin].filledDates.indexOf(date.format('ddd MMM DD YYYY')) !== -1;
            }
    ...
    const availableDates = Array.from(availableRange.by('day'))
        .filter((date) => {
            if (
                ...
                props.bookedDates[cabin].filledDates.indexOf(date.format('ddd MMM DD YYYY')) === -1
            ) {
                return true;
            } else {
                return false;
    ...
    if (props.bookedDates[cabin].filledDates) {
        ...
        if (
            range.from &&
            !range.to &&
            disabledDates.getDate() - range.from.getDate() < props.bookedDates[cabin].maxNights &&
            disabledDates.getMonth() === range.from.getMonth()
        ) {
            const difference = range.from.getDate() - disabledDates.getDate() + props.bookedDates[cabin].maxNights - 1;
    ...
    const priceBreakDown = (from, to) => {
        ...
        if (cabinData.price.length > 1) {
            prices = cabinData.price;
        } else {
        ...
        return (
            <div className="border">
                <h5 className="DarkBlue Font4 my-3">Price Breakdown</h5>
                <div className="Raised px-5 py-3 mb-3" style={{ width: '300px', margin: 'auto' }}>
                    <ObjectTable price object={jsxObj} />
                </div>
            </div>
        );
    };
    ...
    if (range.from && range.to) {
        total = priceBreakDown(range.from, range.to);
    }

    return (
        <div className="ReservationDates">
            <h3>
                Date and Duration: <span className="BrightBlue">{cabinData.name}</span>{' '}
            </h3>
            {props.loading ? (
                <Loader />
            ) : props.fetchError ? (
                ...
                >
                    <ErrorAlert error={props.fetchError} />
                </Modal>
            ) : (
                <React.Fragment>
                    <small className="DarkBlue d-block">
                        <span className="Bungee mr-1" style={{ color: 'hsl(8.3,88.6%,50.4%)' }}>
                            Note:{' '}
                        </span>
                        {'  '} You will checkout the day <span style={{ color: 'hsl(8.3,88.6%,50.4%)' }}>AFTER</span>{' '}
                        your final selected date
                    </small>
                    <DayPicker
                    ...
                    <div className="px-5 pt-3">
                        ...
                        <div
                            className="Info Grey-BG py-3"
                            style={{ maxHeight: range.from && range.to ? '700px' : '100px' }}
                        >
                            {!range.from && !range.to && (
                                <p className="Font4 m-0 p-0 Blue Bungee TextOutline">Please select the first day.</p>
                            )}
                            {range.from && !range.to && (
                                <p className="Font4 m-0 p-0 Blue Bungee TextOutline">Please select the last day.</p>
                            )}
                            {range.from && range.to && (
                                <React.Fragment>
                                    <h5 className="Font4 DarkBlue">Selected Dates</h5>{' '}
                                    <p>
                                        Arrival:{' '}
                                        <span className="BrightBlue Font4">{range.from.toLocaleDateString()}</span>
                                    </p>
                                    <p>
                                        {' '}
                                        Checkout:{' '}
                                        <span className="BrightBlue Font4">
                                            {new Date(Date.parse(range.to) + 60000 * 60 * 24).toLocaleDateString()}
                                        </span>
                                    </p>
                                    {total}
                                </React.Fragment>
...
const mapDispatchToProps = (dispatch) => {
    return {
        onFetchAvailableDates: (hutName, token) => dispatch(actions.fetchAvailability(hutName, token)),
