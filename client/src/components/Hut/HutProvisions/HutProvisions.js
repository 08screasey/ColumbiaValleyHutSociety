...
const HutProvisions = (props) => {
    const provisions = ['Mattresses', 'Bedding', 'Cookware', 'Stove', 'Wifi', 'Power', 'Firewood', 'Water', 'Lantern'];
    const components = [
    ...
    const provisionList = provisions.map((key, i) => {
        const classes = ['Provision', 'd-flex', 'justify-content-around', 'align-items-center', 'mx-2'];
    ...
    const prohibitedList = Object.keys(prohibited).map((key, i) => {
        const classes = ['Prohibited', 'd-flex', 'justify-content-around', 'align-items-center', 'mx-2'];
    ...
    return (
        <React.Fragment>
            <div className="Raised p-2">
                ...
                <ul className="ProvisionList d-flex flex-wrap justify-content-center p-0">{provisionList}</ul>
            </div>
            <div className="Raised Red-BG p-2 my-3 w-75 mx-auto">
                <h4 className="Red Font4 mt-3">Prohibited Items</h4>
                <ul className="d-flex ProhibitedList flex-wrap justify-content-center p-0">{prohibitedList}</ul>
            </div>
        </React.Fragment>
    );
};
