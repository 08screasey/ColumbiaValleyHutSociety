...
const Home = (props) => {
    ...
    return (
        <div className="Home">
            <CSSTransition
                classNames="SignUpAnimation"
                in={props.authenticated && !props.verified && !acknowledged}
                unmountOnExit
                timeout={800}
            >
                <SignUpAlert
                    resend={() => props.onSendVerification(props.userId, props.token)}
                    closeAlert={() => setAcknowledged(true)}
                />
            </CSSTransition>
