...
const CreateUpdate = (props) => {
    ...
    const handleFormSubmit = (e) => {
        e.preventDefault();
        const post = { header: form.header.value, content: form.content.value, comments: [] };
        props.onCreateUpdate(post, props.token);
    };

    return (
        <div className="CreateUpdate Grey-BG">
            {props.error ? (
                <Modal small error closeModal={() => props.onClearError()}>
                ...
                >
                    <div className="text-center">
                        ...
                        <p className="mx-auto my-3">Update successfully created.</p>
                    </div>
                </Modal>
            ) : null}
            ...
            <form onSubmit={handleFormSubmit}>
                <div className="FormGroup">
                    <input
                        type="text"
                        className={form.header.touched && !form.header.valid ? 'FormControl Invalid' : 'FormControl'}
                        placeholder="Header"
                        value={form.header.value}
                        onChange={(e) => handleInputChange(e, 'header')}
                    />
                    {form.header.touched && !form.header.valid ? (
                        <p className="Red d-block text-center m-auto Font6">Please enter a valid header</p>
                    ) : null}
                </div>
                <div className="FormGroup">
                    <textarea
                        rows="10"
                        className={form.content.touched && !form.content.valid ? 'FormControl Invalid' : 'FormControl'}
                        placeholder="Content"
                        value={form.content.value}
                        onChange={(e) => handleInputChange(e, 'content')}
                    />
                    {form.content.touched && !form.content.valid ? (
                        <p className="Red d-block text-center m-auto Font6">Content required.</p>
                    ) : null}
                </div>
                <button className="Button Grey-BG DarkBlue Bungee" disabled={!validity}>
                    {' '}
                    Submit{' '}
                </button>
            </form>
...
const mapStateToProps = (state) => {
    return { token: state.auth.token, error: state.news.error, update: state.news.newUpdate };
};
