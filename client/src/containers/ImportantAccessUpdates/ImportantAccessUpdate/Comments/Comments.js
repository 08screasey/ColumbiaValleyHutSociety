...
const Comments = (props) => {
    ...
    const handleInputChange = (e, identifier) => {
        ...
        newForm[identifier].valid = checkValidity(newForm[identifier].validity, newForm[identifier], form);
        let validForm = true;
        for (let key in form) {
    ...
    return (
        <div
            className="Raised text-left rounded m-auto"
            style={{ width: '95%', maxWidth: '800px', padding: '20px 5%' }}
        >
            {!!props.commentAdded ? (
                <Modal
                    small
                ...
                >
                    <div className="text-center">
                        ...
                        <p className="mx-auto my-3">Comment successfully submitted</p>
                    </div>
                </Modal>
            ) : props.error ? (
                <Modal
            ...
            <form className="text-center" onSubmit={(e) => handleFormSubmit(e)}>
                ...
                >
                    <div className="FormGroup position-relative">
                        ...
                        >
                            <FontAwesomeIcon icon={faTimesCircle} className="Red m-auto" size="1x" />
                        </span>
                        <textarea
                            required
                            rows="6"
                        ...
                        <small
                            style={{
                                color: form.content.value.length > 350 ? 'red' : 'grey',
                            }}
                        >
                            {form.content.value.length}/350
                        </small>
                        {form.content.touched && !form.content.valid && form.content.value.length > 0 ? (
...
const mapDispatchToProps = (dispatch) => {
    return {
        onFormSubmit: (formData, id, token) => dispatch(actions.addComment(formData, id, token)),
        onDeleteComment: (token, userId, id) => dispatch(actions.deleteComment(token, userId, id)),
