...
const ImportantAccessUpdate = (props) => {
	...
	return (<div style={{"paddingTop":"100px"}} className="pb-4 Grey-BG">
		...
		{adminEdit ? <Modal big closeModal={()=>{
			...
			<EditUpdate closeEdit={()=>{setAdminEdit(false);
				props.onClearUpdate()}} form={form} disabled={!validForm} formSubmit={handleFormSubmit} inputChange={(event, identifier)=>handleInputChange(event, identifier)}/>
		</Modal> : props.updated ?
		...
		: adminDelete ?
		 <Modal small error touchedBackdrop={()=>setAdminDelete(false)} closeModal={()=>{
		 	setAdminDelete(false);
		 	props.onDeleteNews(id, props.userData, props.token)
		 }}>
		 	<ErrorAlert error={{message:"Yes, delete this post and all of the comments associated with it.", status:"Are You Sure?"}}/>
		 </Modal> : null}
		<span onClick={()=>setAdminEdit(true)} className="text-right">
		<FontAwesomeIcon
							icon={faPencilAlt}
							className="BrightBlue text-right mr-5"
							size="1x"
						/>
		</span>
		...
		{article.content.split(/\n/).map((paragraph, key)=>{
			...
			return <p className={classes.join(" ")} key={key}>{paragraph}</p>
		})}
		</div>
		</div>
