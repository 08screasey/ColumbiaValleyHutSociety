            ...
            {props.deleted ? (
                ...
                >
                    <div className="text-center">
                        ...
                        <p className="mx-auto my-3">Booking successfully cancelled</p>
                    </div>
                </Modal>
            ) : null}
            {props.loading ? (
                ...
                <React.Fragment>
                    <p>
                        Are you sure you want to cancel this reservation? Cancelling will trigger an automatic refund of{' '}
                        {'$' + (parseFloat(bookingToDelete.price) * 0.8).toFixed(2)} to the account of{' '}
                        {bookingToDelete.userData.email}.
                    </p>
                    ...
                    <button
                        className="Button Red-BG Red Bungee"
                        onClick={() => props.onDeleteReservation(bookingToDelete._id, props.token)}
                    >
                        Yes, cancel this reservation
                    </button>
                    <button className="Button Grey-BG DarkBlue Bungee" onClick={() => props.history.push('/admin')}>
                        No, go back
                    </button>
                </React.Fragment>
            )}
