...
const EditReservation = (props) => {
	...
	const handleSubmit = () => {
		props.onEditReservation(bookingToEdit._id, {dates:dates, userData:{name:form.name.value, email:form.email.value, mobile:form.mobile.value}}, props.token)
		setProceed(false)
	}


	return (<div className="EditReservation">

			{proceed ? <Modal big closeModal={()=>setProceed(false)}>
				<BookingReview editing bookingDetails={{dates:dates, _id:bookingToEdit._id, userData:{name:form.name.value, email:form.email.value, mobile:form.mobile.value}}}/>
				<button onClick={handleSubmit}>Submit</button>
			</Modal>
			:null }
			{props.queryStatus === "Editing" ? <Modal small success closeModal={()=>{props.onClearQueryStatus(); 
			...
			</Modal> : props.selectError ? <Modal small error closeModal={()=>props.onClearError('selectError')}>
	...
	<ReservationDates admin editing={bookingToEdit} onUpdateBooking={(date1, date2)=>{handleUpdateDates(date1, date2)}}/>
</div>)
};
