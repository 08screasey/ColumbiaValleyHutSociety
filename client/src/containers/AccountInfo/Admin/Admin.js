            ...
            <Switch>
                <Route path="/admin" exact>
                    ...
                    <div className="SearchResults">
                        {props.loading ? (
                            <Loader />
                        ) : props.queryResults && props.queryResults.length > 0 ? (
                            props.queryResults.map((result, id) => {
                                return (
                                    <SearchResult
                                        booking={result}
                                        key={id}
                                        editClicked={() => handleEditClicked(result._id, result.hut)}
                                        deleteClicked={() => handleDeleteClicked(result._id, result.hut)}
                                    />
                                );
                            })
                        ) : (
