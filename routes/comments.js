...
router.put('/:id', auth, verifyUser, (req, res) => {
    Comment.create({ ...req.body, articleId: req.params.id }, (err, comment) => {
        if (err) {
                ...
                news.save((err, news) => {
                    if (err) {
                        ...
                        User.findById(req.user.id, (err, user) => {
                            if (err) {
                                res.status(500).send('unable to store comment in user profile', err);
                            } else {
                                user.comments.push(comment);
                                user.save((err) => {
                                    if (err) {
                                        res.status(500).send('there was an error saving this user');
                                    } else {
                                        res.status(200).json({
                                            comment: comment,
...
router.delete('/:postId/:commentId', auth, verifyUser, (req, res) => {
    News.findById(req.params.postId)
        ...
        .exec((err, news) => {
            ...
            } else if (req.user.id !== comment.userId && req.user.id !== process.env.CVHS_ADMIN_ID) {
            ...
            news.save((err) => {
                if (err) {
                    ...
                    User.findById(req.user.id)
                        ...
                        .exec((err, user) => {
                            if (err) {
                                res.status(500).send('There was an error fetching user' + req.user.id);
                            } else {
