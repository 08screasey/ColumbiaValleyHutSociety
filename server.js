...
paypal.configure({
  ...
  'client_id': "AcpVrNSnkzIKYfkYidDROb4mi3J51puEKMnmpomsxdbVO-KRyYY45yPCd2-lYkx8NJDDoF2x49KQQTKN",
  'client_secret': "EKy3FPw9wjIGiIZlVcFGvWR2uaMwLDJuqWLRIZVKZW-_de4pTvlViFPOshpxk3GuVeq3kLz1JB7TNksh"
});
...
newsRoutes.put("/:id", auth, (req, res) => {
	...
	News.findByIdAndUpdate(req.params.id, req.body.post, {new:true}, (err, news) => {
		console.log(news)
		if (err || !news) {
		...
		(err, comment) => {
			if (err) {
				...
				News.findOne({ _id: req.params.id }, (err, news) => {
					...
					news.save((err, news) => {
						if (err) {
							res.status(500).send("Unable to save comment to article")
						} else {
...
commentRoutes.delete("/:postId/:commentId", auth, verifyUser, (req, res) => {
	News.findById(req.params.postId)
		...
		.exec((err, news) => {
			...
			}else if(req.user.id !== comment.userId && req.user.id !=="5f32e85f17bc722350c0bdca"){
				res.status(401).send("You do not have permission to delete this comment");
			}
			news.comments.remove({ _id: req.params.commentId });
			news.save((err) => {
				if (err) {
					res.status(500).send("There was an error pulling the comment");
				} else {
					User.findById(req.user.id)
						...
						.exec((err, user) => {
							if (err) {
								...
								user.save((err) => {
									if(err){
										res.status(500).send("Unable to save user, please try again.")
									}									
...
bookingRoutes.delete("/search/:id", auth, (req, res)=>{
	...
	Booking.findByIdAndRemove(req.params.id, (err,booking)=>{
		if(err){
			...
			Hut.findOne({name: booking.hut}).populate('bookings').exec((err,hut)=>{
				if(err){
	...
	Booking.findById(req.params.id, (err,booking)=>{
		if(err || !booking){
			...
			Hut.findOne({name:booking.hut}, (err,hut)=>{
				...
				else {console.log(booking.dates, "PRE")
					booking.dates.forEach(date=>{
						hut.filledDates.pull(date)
					})
...
bookingRoutes.post("/:hut", auth, verifyUser, (req, res) => {
    ...
    "redirect_urls": {
        "return_url": "http://localhost:3000/reservations/success",
    ...
    if (error) {
    	...
    	for (let i=0; i<payment.links.length; i++){
    		if(payment.links[i].rel === "approval_url"){
    				res.status(200).json({ name: hut.name, dates: dates, paypalLink:payment.links[i].href });
    		}
    	}
...
bookingRoutes.post("/:hut/book", auth, verifyUser, (req, res) => {
	let slicedDates = req.body.dates.map(date=>{return moment(date).format('ddd MMM DD YYYY')})
    ...
    if (error) {
        ...
        Booking.create(
				{ ...req.body, dates: slicedDates, hut:req.params.hut },
				(err, booking) => {
					if (err) {
						...
						Hut.findOne({ name: req.params.hut }, (err, hut) => {
							...
							hut.save((err, hut) => {
								if (err) {
									...
									User.findById(req.user.id, (err, user)=>{
										if(err){
											res.status(500).send("Cannot locate user with Id " + req.user.id)
										}else{
											user.reservations.push(booking);
											user.save(err=>{
												if(err){
													res.status(500).send("Unable to save user " + req.user.id);
												}else{
													...
													hut[0] = hut[0].toUpperCase();
														...
														const msg = {
														  ...
														  dynamic_template_data:{
														  	...
														  	"date":new Date(booking.dates[0]).toLocaleDateString(),
														  	"date2":new Date(booking.dates[booking.dates.length-1]).toLocaleDateString(),
